...
cavab_values = {}
async def cavab_changer(value, chat_id):
    cavab_values[chat_id] = value
...
async def sender(event):
    ...
    for key, value in data.items():
        await cavab_changer(value, event.chat.id)
...
async def show_result(event):
    ...
    # Load JSON file
    with open(f"users_{chat_id}.json", "r", encoding="utf-8") as users_file:
        user_data = json.load(users_file)

    os.remove(f"users_{chat_id}.json")
...
async def check(event: events.newmessage.NewMessage.Event):
    if not event.is_private and not await is_valid_url(event.text):
        try:
            if event.text.lower() == cavab_values[event.chat.id][7:].lower() or event.text in meyar:
                user_data = []
                chat_id = event.chat.id
                if not os.path.exists(f"users_{chat_id}.json"):
                    with open(f"users_{chat_id}.json", "w", encoding="utf-8") as users_file:
